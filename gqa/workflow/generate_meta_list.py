...
if __name__ == '__main__':
    ...
    try:
        with open(f'{args.meta}/full_vocab.json', 'r') as f:
            vocab = json.load(f)

        with open(f'{args.meta}/answer_vocab.json', 'r') as f:
            answer = json.load(f)
    ...
    basic_kwargs = dict(
        length=args.length,
        object_info=os.path.join(args.meta, args.object_info),
    ...
    dataset = GQA(
        split='submission',
        mode='val',
        contained_weight=args.contained_weight,
